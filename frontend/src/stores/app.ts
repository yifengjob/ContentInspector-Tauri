...
export const useAppStore = defineStore('app', () => {
  ...
  function getNodeCheckState(
    nodePath: string,
    allNodes: Map<string, DirectoryNode>
  ...
  function getEffectiveScanPaths(): string[] {
    ...
    for (const path of paths) {
      ...
      const hasDescendantSelected = paths.some(
        (otherPath) => otherPath !== path && otherPath.startsWith(path + separator)
      );
  ...
  return {
    ...
    flushPendingResults, // 【关键修复】导出强制flush方法
    addLog, // 单条添加日志
    addLogs, // 【新增】批量添加日志
    clearScanResults,
    removeResult,
